...
def train(
    ...
    use_wandb = opt.use_wandb and WANDB_AVAILABLE and opt.is_main
    if use_wandb:
    ...
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(data_iterator):
            ...
            if step % opt.accumulation_steps == 0 and not stats["skip_example"]:
                ...
                # Clear CUDA cache every 50 steps to prevent memory accumulation
                if step % 50 == 0:
            ...
            if step % opt.log_freq == 0:
                ...
                if use_wandb:
                    wandb_metrics["lr"] = scheduler.get_last_lr()[0]
                    wandb_metrics["memory_gb"] = torch.cuda.max_memory_allocated() / 1e9
            ...
            if step % opt.eval_freq == 0:
                for data_path in opt.eval_data:
                    ...
                    wandb_eval_metrics = {} if use_wandb else None
                    ...
                    for k, v in metrics.items():
                        ...
                        if use_wandb:
                            wandb_eval_metrics[f"eval/{dataset_name}/{k}"] = v
...
if __name__ == "__main__":
    ...
    if opt.use_wandb and opt.is_main:
        if not WANDB_AVAILABLE:
            ...
            wandb_run_name = opt.wandb_run_name if opt.wandb_run_name else opt.name
            wandb_config = {
                "learning_rate": opt.lr,
                "batch_size": opt.per_gpu_batch_size,
                "total_steps": opt.total_steps,
                "model": opt.model_path,
                "use_grpo": getattr(opt, 'use_grpo', False),
                "grpo_group_size": getattr(opt, 'grpo_group_size', None),
                "grpo_train_retriever": getattr(opt, 'grpo_train_retriever', False),
                "grpo_retriever_group_size": getattr(opt, 'grpo_retriever_group_size', None),
                "grpo_kl_coeff": getattr(opt, 'grpo_kl_coeff', None),
                "grpo_clip_epsilon": getattr(opt, 'grpo_clip_epsilon', None),
            }
            wandb.init(
