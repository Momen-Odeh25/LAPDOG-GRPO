...
def load_reader(opt):
    ...
    if not opt.retrieve_only:
        # Use local flan-t5-base if specified
        if opt.reader_model_type == "google/flan-t5-base":
            local_model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "flan-t5-base"))
            reader = src.fid.FiD.from_pretrained(local_model_path)
            reader_tokenizer = transformers.AutoTokenizer.from_pretrained(local_model_path, use_fast=False)
        else:
...
def _load_lapdog_model_state(opt, opt_checkpoint, model, model_dict):
    ...
    # Filter out reference_reader keys (for GRPO) - these are initialized separately
    model_dict = {k: v for k, v in model_dict.items() if not k.startswith("reference_reader")}
